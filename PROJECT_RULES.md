...
4. **Do not implement voice STT/TTS or conversation state machines in this repo** unless a ticket explicitly moves that ownership here. Prefer the backend `voice-agent` repository. **Exceptions:** (KAN-10) browser microphone permission, MediaStream capture, level metering, and PCM chunk handoff; (KAN-16) MediaRecorder → base64 clips, `POST /api/voice/turn` / session HTTP wiring, and TTS playback of backend `audioBase64`. See [`docs/voice/AUDIO_CAPTURE.md`](docs/voice/AUDIO_CAPTURE.md) and [`docs/architecture/VOICE_PIPELINE.md`](docs/architecture/VOICE_PIPELINE.md).
